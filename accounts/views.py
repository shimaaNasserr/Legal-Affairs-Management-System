...
class UserViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        if getattr(self.request.user, "role_name", None) not in ['President', 'GeneralManager']:
            raise PermissionDenied("فقط الرئيس أو المدير العام يمكنهما إنشاء مستخدمين جدد")
        serializer.save()

    def perform_update(self, serializer):
        if getattr(self.request.user, "role_name", None) not in ['President', 'GeneralManager']:
            raise PermissionDenied("فقط الرئيس أو المدير العام يمكنهما تعديل المستخدمين")
        serializer.save()
